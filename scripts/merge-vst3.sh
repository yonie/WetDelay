...
# Copy Windows binary
# Artifacts are uploaded with trailing / so contents go directly into artifacts/windows/
...
if [ -f "artifacts/windows/Contents/x86_64-win/WetDelay.dll" ]; then
    ...
    echo "  Found at artifacts/windows/VST3/Release/WetDelay.vst3/Contents/x86_64-win/WetDelay.dll"
else
    echo "WARNING: Windows binary not found!"
    echo "Looking for Windows DLL..."
...
if [ -f "artifacts/macos/Contents/MacOS/WetDelay" ]; then
